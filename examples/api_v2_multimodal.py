...
async def main():
    ...
    parser.add_argument(
        "--agent", action="store_true", help="Use an agent for more complex reasoning"
    )
    ...
    if args.agent:
        ...
        print_colorful(
            "Creating Multimodal Agent", "Setting up a multimodal-capable agent", "blue"
        )
        ...
        try:
            agent_response = await call_api(
                "POST", "/v2/agents/create", agent_request, API_KEY
            )
            ...
            if args.stream:
                ...
                async with httpx.AsyncClient() as client:
                    async with client.stream(
                        "POST", url, json=run_request, headers=headers
                    ) as response:
                ...
                # Run the agent
                run_response = await call_api(
                    "POST", f"/v2/agents/{agent_id}/run", run_request, API_KEY
                )
        ...
        try:
            print_colorful(
                ...
                f"Processing image with prompt: {args.prompt}",
                "blue",
            )
            ...
            response = await call_api(
                "POST", "/v2/completions", generation_request, API_KEY
            )

            print_colorful(
                "Multimodal Analysis", response["choices"][0]["text"], "green"
            )
