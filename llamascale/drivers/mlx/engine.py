...
class MLXModelEngine:
    ...
    def __init__(self, model_path: str, config: Union[Dict[str, Any], MLXConfig]):
        ...
        if self.mlx_available:
            ...
            self.kv_cache = (
                self._init_kv_cache() if self.config.kv_cache_config else None
            )
        else:
            self.model = None
    ...
    def _init_kv_cache(self):
        ...
        return {"max_seq_len": max_seq_len, "cache": {}, "enabled": True}
    ...
    async def generate(
        ...
        if self.mlx_available:
            ...
            output_ids = self._mock_generate_tokens(input_ids, max_tokens=max_tokens)
    ...
    async def generate_stream(
        ...
        yield {"type": "start", "usage": {"prompt_tokens": len(input_ids)}}
