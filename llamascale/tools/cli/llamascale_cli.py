...
class LlamaScaleCLI:
    ...
    def _setup_parser(self) -> argparse.ArgumentParser:
        ...
        # Generate command
        generate_parser = subparsers.add_parser("generate", help="Generate text from LLM")
        generate_parser.add_argument("--model", "-m", required=True, help="Model to use")
        ...
        generate_parser.add_argument("--top-p", type=float, default=0.9, help="Top-p sampling parameter")
        generate_parser.add_argument("--seed", type=int, help="Random seed for reproducibility")
        generate_parser.add_argument("--output", "-o", help="Output file path (stdout if not specified)")
        generate_parser.add_argument("--stream", action="store_true", help="Stream output tokens")
        ...
        download_parser.add_argument("--model", "-m", required=True, help="Model to download")
        download_parser.add_argument("--force", "-f", action="store_true", help="Force redownload if model exists")
        ...
        cache_clear_parser.add_argument("--all", "-a", action="store_true", help="Clear all caches")
        cache_clear_parser.add_argument("--memory", action="store_true", help="Clear memory cache")
        cache_clear_parser.add_argument("--disk", action="store_true", help="Clear disk cache")
        ...
        benchmark_parser.add_argument("--tokens", "-t", type=int, default=1024, help="Number of tokens to generate")
        benchmark_parser.add_argument("--runs", "-r", type=int, default=3, help="Number of benchmark runs")
        
        # Init command
        init_parser = subparsers.add_parser("init", help="Initialize LlamaScale configuration")
            ...
            if self.args.detailed:
                ...
                if os.path.exists(config_path):
                    try:
                        ...
                        print(f"      Context Length: {config.get('max_seq_len', 'unknown')}")
                    except Exception as e:
    ...
    def _handle_download(self):
        ...
        if os.path.exists(model_path) and not self.args.force:
            ...
            print(f"Model {model_name} already exists. Use --force to redownload.", file=sys.stderr)
            return 1
    ...
    async def _handle_benchmark(self):
        ...
        try:
            ...
            for i in range(runs):
                ...
                print(f"  - Time: {run_time:.2f}s, Tokens: {gen_tokens}, Tokens/sec: {tokens_per_second:.2f}")
    ...
    async def run(self) -> int:
        ...
        elif self.args.command in ["config-init", "config-view", "config-preset", 
                                   "config-set", "config-add-model", "config-remove-model"]:
    ...
    def _print_llama_fact(self):
        ...
        if HAS_RICH:
            console.print(Panel(f"[yellow]🦙 Llama Fact:[/yellow] [cyan]{fact}[/cyan]", 
