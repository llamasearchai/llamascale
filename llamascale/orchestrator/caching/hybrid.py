...
class HybridCache:
    ...
    async def get(self, 
                key: str, 
                model: str = "default") -> Optional[Any]:
        ...
        if self.redis:
            try:
                ...
                if redis_value:
                    ...
                    # Check TTL
                    if "expiry" not in value_dict or value_dict["expiry"] is None or time.time() < value_dict["expiry"]:
                        # Cache in memory for future
                        self.memory[hashed_key] = (value_dict["value"], value_dict.get("expiry"))
    ...
    async def set(self, 
        ...
        """
        hashed_key = self._hash_key(key)
        expiry = time.time() + (ttl or self.ttl) if ttl is not None or self.ttl is not None else None
        ...
        if self.redis:
            try:
                await self.redis.set(
                    ...
                    ex=ttl or self.ttl if ttl is not None or self.ttl is not None else None
                )
            except Exception as e:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if self.disk_path:
            try:
                disk_files = [f for f in os.listdir(self.disk_path) if f.endswith(".json")]
                stats["disk_items"] = len(disk_files)
                stats["disk_size"] = sum(os.path.getsize(os.path.join(self.disk_path, f)) for f in disk_files)
            except Exception as e:
