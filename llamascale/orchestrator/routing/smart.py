...
class BackendNode:
    ...
    def can_handle(self, model: str, batch_size: int = 1) -> bool:
        ...
        return (
            ...
            and self.health_status != NodeHealth.UNHEALTHY
            and self.current_load + batch_size <= self.capacity.get("max_batch_size", 1)
        )
    ...
    def get_expected_latency(self, model: str, tokens: int) -> float:
        ...
        if model in self.last_latency:
            ...
            load_factor = 1.0 + (
                self.current_load / self.capacity.get("max_batch_size", 1)
            )
    ...
    def update_metrics(
        self, model: str, latency: float, tokens: int, success: bool = True
    ):
        """Update node metrics after request completion"""
        # Update latency metrics with exponential moving average
        alpha = 0.2  # Smoothing factor
        if model in self.last_latency:
            self.last_latency[model] = (1 - alpha) * self.last_latency[
                model
            ] + alpha * latency
        else:
...
class AdaptiveRoutingStrategy:
    ...
    def __init__(self, weights: Dict[str, float] = None):
        ...
        """
        self.weights = weights or {"latency": 0.6, "cost": 0.3, "reliability": 0.1}
    ...
    def select_node(
        ...
        for node in capable_nodes:
            ...
            cost_score = (
                node.cost_per_token / max(n.cost_per_token for n in capable_nodes)
                if max(n.cost_per_token for n in capable_nodes) > 0
                else 0
            )
...
class InferenceRouter:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.strategy = AdaptiveRoutingStrategy(weights=config.get("routing_weights"))
        self.request_queues = {}
        self.queue_tasks = []
        self.engines = {}
            ...
            try:
                ...
                for model in mlx_models:
                    ...
                    backends["mlx"][model] = MLXModelEngine(
                        model_path=model_path, config=mlx_config
                    )

                logger.info(
                    f"MLX backend initialized with models: {list(backends['mlx'].keys())}"
                )
            except ImportError:
    ...
    async def _process_queue(self, priority):
        ...
        while True:
            ...
            logger.debug(
                f"Request dequeued after {queue_time:.3f}s with priority {priority}"
            )
            ...
            try:
                ...
                if node is None:
                    raise ValueError(
                        f"No suitable backend found for model {request.get('model')}"
                    )
                ...
                if node:
                    node.update_metrics(
                        model=request.get("model"), latency=0, tokens=0, success=False
                    )
            finally:
                # Update node load
                if node:
...
class CostTracker:
    ...
    def get_stats(self):
        ...
        return {
            ...
            "avg_cost_per_token": (
                self.total_cost / self.token_count if self.token_count > 0 else 0
            ),
        }
