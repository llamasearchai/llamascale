...
class GenerationRequest(BaseModel):
    ...
    priority: Optional[int] = Field(RequestPriority.NORMAL, description="Request priority (0-3)")
...
async def startup_event():
    ...
    logger.info(f"API server will listen on {llamascale_config.api_host}:{llamascale_config.api_port}")
...
async def get_server_config():
    ...
    safe_config = {
        "mlx_config": {k: v for k, v in config["mlx_config"].items() if k not in ["api_keys"]},
        "cache_config": {k: v for k, v in config["cache_config"].items() if k not in ["auth_token"]},
