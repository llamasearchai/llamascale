...
class GenerationRequest(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        100, description="The maximum number of tokens to generate"
    )
    ...
    tools: Optional[List[ToolDefinition]] = Field(
        None, description="The tools available to the model"
    )
    tool_choice: Optional[Union[str, ToolChoice]] = Field(
        None, description="Which tool to use"
    )
    priority: Optional[RequestPriority] = Field(
        RequestPriority.NORMAL, description="Priority for request processing"
    )
    images: Optional[List[ImageInput]] = Field(
        None, description="Images for multimodal processing"
    )
...
class StreamingResponse(BaseModel):
    ...
    tool_calls: Optional[List[Dict[str, Any]]] = Field(
        None, description="Any tool calls made by the model"
    )
...
class ModelInfo(BaseModel):
    ...
    type: Optional[str] = Field(
        "llm", description="The model type (llm, embedding, etc.)"
    )
    parameters: Optional[int] = Field(
        None, description="The number of parameters in the model"
    )
    context_length: Optional[int] = Field(
        None, description="The maximum context length"
    )
    quantization: Optional[str] = Field(
        None, description="The quantization method used"
    )
    capabilities: Optional[List[str]] = Field(
        None, description="The model's capabilities"
    )
    supports_multimodal: Optional[bool] = Field(
        False, description="Whether the model supports multimodal inputs"
    )
...
class AgentAction(BaseModel):
    ...
    tool_calls: Optional[List[ToolCall]] = Field(
        None, description="Any tool calls made during the action"
    )
...
class AgentRequest(BaseModel):
    ...
    max_steps: Optional[int] = Field(
        10, description="Maximum number of steps the agent can take"
    )
    tools: Optional[List[ToolDefinition]] = Field(
        None, description="Tools available to the agent"
    )
    system_prompt: Optional[str] = Field(
        None, description="System prompt to override agent's default"
    )
    images: Optional[List[ImageInput]] = Field(
        None, description="Images for multimodal processing"
    )
...
class HealthStatus(BaseModel):
    ...
    components: Optional[Dict[str, bool]] = Field(
        None, description="Status of API components"
    )
    models_loaded: Optional[List[str]] = Field(
        None, description="Models currently loaded in memory"
    )
