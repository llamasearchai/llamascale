...
from .schemas import ToolCall, ToolDefinition, FunctionDefinition
...
class ToolRegistry:
    ...
    def register(self, func: Callable, name: Optional[str] = None, description: Optional[str] = None) -> None:
        ...
        for param_name, param in sig.parameters.items():
            ...
            if (param.default is not inspect.Parameter.empty and 
                isinstance(param.default, Field)):
                parameters["properties"][param_name]["description"] = param.default.description
    ...
    def _type_to_json_schema(self, typ: Type) -> Dict[str, Any]:
        ...
        if origin is Union:
            ...
            return {
                "oneOf": [self._type_to_json_schema(arg) for arg in args]
            }
...
async def get_current_weather(
    ...
    unit: str = Field("celsius", description="The temperature unit to use: 'celsius' or 'fahrenheit'")
) -> Dict[str, Any]:
...
def calculate(expression: str = Field(..., description="The mathematical expression to evaluate")) -> Dict[str, Any]:
    ...
    try:
        # Sanitize input
        if any(keyword in expression for keyword in ['__', 'import', 'eval', 'exec', 'open']):
