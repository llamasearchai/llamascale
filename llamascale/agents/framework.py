...
class ToolResult(BaseModel):
    ...
    meta: Dict[str, Any] = Field(default_factory=dict, description="Metadata about execution")
...
class Tool(BaseModel):
    ...
    async_handler: Callable = Field(..., description="Async function that implements the tool")
...
class ImageTool(Tool):
    ...
    use_mlx_vision: bool = Field(default=True, description="Whether to use MLX Vision for image analysis")
    
    async def analyze_image(self, image_path: str, analysis_type: str = "caption") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                has_pil = False
                return ToolResult(success=False, error="PIL not installed. Install with 'pip install pillow'")
...
"""
    ...
    async def _get_next_action(self, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        formatted_messages = [{"role": "system", "content": self.custom_system_prompt or self._default_system_prompt()}]
    ...
    async def _parse_model_response(self, response: str) -> Dict[str, Any]:
        ...
        if self.reasoning_strategy == ReasoningStrategy.REACT:
            ...
            if "Action:" in response and "Action Input:" in response:
                action_match = response.split("Action:")[1].split("Action Input:")[0].strip()
    ...
    async def process_multimodal_input(self, text: str, image_paths: List[str] = None) -> Dict[str, Any]:
        ...
        for idx, result in enumerate(image_analysis_results):
            if "analysis" in result and "caption" in result["analysis"]:
                image_descriptions.append(f"Image {idx+1}: {result['analysis']['caption']}")
            else:
...
"""
    ...
    async def run_stream(self, query: str, max_iterations: int = 10) -> AsyncGenerator[Dict[str, Any], None]:
